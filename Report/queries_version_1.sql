...
join posts p1 ignore index (primary) on p1.id = pt.post_id and p1.accepted_post_id is null
join posts p2 on p2.parent_post_id = p1.id and p2.post_type = 2
...
from posttags pt
straight_join posts p1 use index (primary) on p1.id = pt.post_id and p1.accepted_post_id is null
join users u on u.id = p1.user_id
join votes v on v.post_id = p1.id
straight_join tags t on pt.tag_id = t.id
straight_join posts p2 use index (parent_post_id) on p1.id = p2.parent_post_id and p2.post_type = 2
group by p1.id
